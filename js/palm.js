...
(function(root, factory) {
  ...
  function getViewMarkup() {
    return [
      ...
      '    </div>',
      '  </div>',
      '  <details class="palm-api-guide">',
      '    <summary><span>API 获取教程</span><small>第一次使用 AI 解读前，先申请一个 Gemini API Key</small></summary>',
      '    <div class="palm-api-guide-body">',
      '      <ol class="palm-api-steps">',
      '        <li><strong>打开 Google AI Studio</strong><span>访问 <a href="https://aistudio.google.com/" target="_blank" rel="noopener noreferrer">https://aistudio.google.com/</a>，使用 Google 账号登录。</span></li>',
      '        <li><strong>进入 API 密钥页面</strong><span>在左侧或顶部找到“API 密钥 / API Keys”，点击“创建 API 密钥 / Create API key”。</span></li>',
      '        <li><strong>创建并复制密钥</strong><span>名称可以填写 Gemini API Key，项目选择默认项目即可；创建后点击复制图标，把以 AIza 开头的 Key 复制下来。</span></li>',
      '        <li><strong>回到本站保存</strong><span>点击上方“设置 API Key”，粘贴密钥并保存；之后上传手掌照片，就可以点击“AI 深度解读”。</span></li>',
      '      </ol>',
      '      <p class="palm-api-warning">安全提醒：API Key 相当于你的调用凭证，不要发给陌生人，也不要截图公开。本站只会把它保存在当前浏览器里。</p>',
      '    </div>',
      '  </details>',
